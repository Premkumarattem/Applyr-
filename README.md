# Applyr — Job Search & Direct Email Outreach

Search open roles across several countries, auto-include your resume summary in outreach emails, and send directly to hiring managers and recruiting contacts — all from one place.
...
- **Draft & send outreach** — compose an email per job, auto-populate recipient domain (`careers@domain.com`), optionally look up likely HR/recruiting contacts via [Hunter.io](https://hunter.io/), and automatically include your candidate highlights in the email body. Send via **Resend** or your own SMTP account.
...
4. **(Optional) Get a Hunter.io key** at https://hunter.io/ if you want the "Find contact"
...
5. **Copy the env file and fill in your keys**
   ```bash
   cp .env.example .env
   ```
